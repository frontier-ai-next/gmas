...
class TestSerperProvider:
    ...
    def test_search_images(self):
        ...
        with patch("gmas.tools.web_search._providers._urlopen", return_value=mock_response) as mock_urlopen:
            results = provider.search_images("python logo", max_results=3)

        assert len(results) == 1
        assert results[0]["image_url"] == "https://cdn.example/logo.png"
        assert results[0]["url"] == "https://example.com/page"
        request = mock_urlopen.call_args[0][0]
        assert request.full_url == SerperProvider._ENDPOINTS["images"]
...
class TestTavilyProvider:
    ...
    def test_search_images_with_descriptions(self):
        provider = TavilyProvider(api_key="key")
        tavily_response = {
            "images": [
                {"url": "https://img.example/a.jpg", "description": "Photo A"},
                {"url": "https://img.example/b.jpg", "description": "Photo B"},
            ]
        }
        mock_response = MagicMock()
        mock_response.__enter__ = lambda s: s
        ...
        assert results[0]["title"] == "Photo A"
        ...
        assert body.get("topic") != "images"

    def test_search_images_string_urls(self):
        provider = TavilyProvider(api_key="key")
        tavily_response = {"images": ["https://img.example/one.jpg", "https://img.example/two.jpg"]}
        mock_response = MagicMock()
        mock_response.__enter__ = lambda s: s
        mock_response.__exit__ = MagicMock(return_value=False)
        mock_response.read.return_value = json.dumps(tavily_response).encode("utf-8")
    ...
    def test_search_images_empty_without_results_fallback(self):
        provider = TavilyProvider(api_key="key")
        tavily_response = {
            "results": [{"title": "Web page", "url": "https://example.com", "content": "text"}],
            "images": [],
        }
        mock_response = MagicMock()
        mock_response.__enter__ = lambda s: s
...
class TestBraveProvider:
            ...
            "results": [
                {
                    ...
                    "source": "example.com",
                    "properties": {"url": "https://cdn.example/logo.png"},
                    "thumbnail": {"src": "https://cdn.example/thumb.png"},
                }
            ]
        }
        mock_response = MagicMock()
    ...
    def test_search_images_null_thumbnail_and_properties(self):
        provider = BraveProvider(api_key="brave-key")
        brave_response = {
            "results": [
                {
